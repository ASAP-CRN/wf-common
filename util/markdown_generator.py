...
def generate_markdown_report(
	...
	staging_combined_manifest = file_results[staging]["combined_manifest_df"]
	production_combined_manifest = file_results["curated"]["combined_manifest_df"]
	...
	staging_sample_loc = file_results[staging]["sample_list_loc"].unique()[0]
	production_sample_loc = file_results["curated"]["sample_list_loc"].unique()[0]
	...
	modified_files_rows = "\n".join(f"| {filename} | {info["staging_hash"]} | {info["curated_hash"]} |" 
	...
	**Tests passed:** N/A
	...
	## Modified
	| filename | md5 hash (staging) | md5 hash (prod) |
	|---------|---------|---------|
	{modified_files_rows}
	...
	**Previous manifest:** {previous_manifest_loc}MANIFEST.tsv
	"""
