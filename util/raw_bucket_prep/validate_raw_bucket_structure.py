...
Usage as module:
from validate_raw_bucket_structure import perform_qc
...
def is_critical_issue(issue: str) -> bool:
    ...
    ----------
    issue : str
        Issue string as generated by `perform_qc`.
...
def get_important_warnings(result: dict) -> list[str]:
    ...
    ----------
    result : dict
        QC result dictionary from `perform_qc`.
...
def generate_report(all_results: list, report_path: Path) -> None:
    ...
    all_results : list of dict
        QC result dicts from `perform_qc`.
    report_path : Path
...
def main() -> int:
    ...
    try:
        result = perform_qc(gs_bucket, outdir, save_metadata)
